...
def parse_arp_table(text: str) -> list[DiscoveredHost]:
    ...
    for line in text.splitlines():
        ...
        hosts[ip] = DiscoveredHost(
            ip=ip,
            mac_address=normalize_mac(match.group("mac")),
            interface=current_interface,
            source="arp_table",
            status="observed",
        )
    return sorted(hosts.values(), key=lambda item: tuple(int(part) for part in item.ip.split(".")))


def parse_neighbor_table(text: str) -> list[DiscoveredHost]:
    hosts: dict[str, DiscoveredHost] = {}
    for line in text.splitlines():
        columns = line.split()
        if not columns:
        ...
        interface = (
            columns[columns.index("dev") + 1]
            if "dev" in columns and columns.index("dev") + 1 < len(columns)
            else ""
        )
...
def read_arp_table(config: NetworkSensorConfig | None = None) -> list[DiscoveredHost]:
    config = config or NetworkSensorConfig()
    text = _run_command(["arp", "-a"], timeout=config.command_timeout_seconds)
    return parse_arp_table(text)


def read_neighbor_table(config: NetworkSensorConfig | None = None) -> list[DiscoveredHost]:
    config = config or NetworkSensorConfig()
...
def _merge_hosts(hosts: list[DiscoveredHost]) -> list[DiscoveredHost]:
...
def _read_passive_hosts(config: NetworkSensorConfig) -> list[DiscoveredHost]:
...
def discover_lan_hosts(config: NetworkSensorConfig | None = None) -> list[DiscoveredHost]:
    config = config or NetworkSensorConfig()
    ...
    if not config.passive_only:
        ...
        active_results: list[DiscoveredHost] = []
    ...
    with ThreadPoolExecutor(max_workers=min(config.discovery_workers, max(1, len(targets)))) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                online = False
            if online:
                discovered.append(DiscoveredHost(ip=ip, source="ping_sweep", status="online"))
    return discovered


def _tcp_port_checks(
    target_ips: list[str], local_cidrs: list[str], config: NetworkSensorConfig
) -> list[DiscoveredHost]:
