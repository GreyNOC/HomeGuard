...
- Network Map: a graphical local-device + cloud-node topology (adapted from the GreyNOC saturn mapping engine). Local LAN devices group under the gateway; this host's live external/internet connections show as cloud nodes. Strictly LAN-scoped (VPN tunnel interfaces are excluded from the local network), with zoom/pan, click-to-detail, and collapsed bundles for peripheral/inactive devices so a decent-sized home lab stays readable.
- Per-device cloud edges (opt-in): point HomeGuard at a home-lab router (OpenWrt/DD-WRT in Phase 1) and it reads the router's conntrack table over SSH to map which *other* LAN devices talk to which internet endpoints — not just this host. Off by default, fetched live and never stored, configured via `GNHL flow set`. See `docs/design/PER_DEVICE_CLOUD_MAP.md`.
